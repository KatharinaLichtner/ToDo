...
class Window(QtWidgets.QWidget):
    ...
    def getPressedButton(self, ev):
        ...
        # after releasing the "A"-Button on the Wiimote the function of the pressed button is called
        if self.wiimote.buttons["A"]:
            self.buttonA = True
            self.app.postEvent(self.toDoList, QtGui.QMouseEvent(QtGui.QMouseEvent.MouseButtonPress,
                                                                QtCore.QPoint(x, y), QtCore.Qt.LeftButton,
            ...
            if self.buttonA is True:
                ...
                self.app.postEvent(self.toDoList, QtGui.QMouseEvent(QtGui.QMouseEvent.MouseButtonRelease,
                                                                    QtCore.QPoint(x, y), QtCore.Qt.LeftButton,
                                                                    QtCore.Qt.NoButton, QtCore.Qt.NoModifier))
                # if the pointer is on the undo-Button the last action will be undone
                if x >= xUndo:
                    if x <= xUndo + self.undoButton.width():
                        if y >= yUndo:
                            if y <= yUndo + self.undoButton.height():
                                self.buttonA = False
                                self.undo()
                # if the pointer is on the redo-Button the last action will be redone
                if x >= xRedo:
                    if x <= xRedo + self.redoButton.width():
                        if y >= yRedo:
                            if y <= yRedo + self.redoButton.height():
                                self.buttonA = False
                                self.redo()
                # if the delete button is pressed the selected item will be released
                if x >= xDelete:
                    if x <= xDelete + self.deleteButton.width():
                        if y >= yDelete:
                            if y <= yDelete + self.deleteButton.height():
                                self.buttonA = False
                                self.delete()
                # if the add button is pressed an item will be added
                if x >= xaddItem:
                ...
                # if the ok button of the edit popup is clicked the editing will be finished
                ...
                # if the cancel button of the edit popup is clicked the editing will be canceled
                if self.editItems.isVisible():
                ...
                # if the ok button in the add item popup is clicked a new item will be added
                ...
                # if the cancel button in the add item popup is clicked the popup will be closed
                if self.inputToDo.isVisible():
        ...
        # if the 'Left'-Button is released the to do tab will be shown
        if self.wiimote.buttons['Left']:
            ...
            if self.tab.currentIndex() is not 0:
                if self.leftButton is True:
                    ...
                    self.leftButton = False
        # if the 'Left'-Button is released the  done tab will be shown
        ...
        # if the 'One'-Button is released the last action will be undone
        if self.wiimote.buttons['One']:
            ...
            if self.undoOne:
                self.undoOne = False
                self.undo()
        # if the 'Two'-Button is released the last action will be redone
        if self.wiimote.buttons['Two']:
            ...
            if self.redoTwo:
                self.redoTwo = False
                self.redo()
        # if the 'Home'-Button is released a website opens
...
# sets an update rate for the accelerometer-sensors to get new data
...
# updates the accelerometer data of the wiimote
...
# sets the updated accelerometer values into a variable
    def update_accel(self, acc_vals):
        self._acc_vals = acc_vals

# getting the infrared data of the wiimote, which will be used for pointing and drawing
# calculating the moving average of the infrared data to filter the shaking of the hand
